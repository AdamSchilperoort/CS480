                  ...
                  char *buffer, Boolean omitLeadingWhiteSpace )
{
    ...
    // loop while character is not stop character & buffer size is available
    while( charAsInt != (int) stopChar && bufferSizeAvailable == True )
    {
        ...
        {
            //assign input to buffer string
            buffer[ charIndex ] = (char) charAsInt;
            charIndex++;
        }
        
        // set next buffer element to null
